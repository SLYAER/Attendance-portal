...
export default function Kiosk({ onOpenAdmin, isAdmin }: KioskProps) {
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!codesDoc.exists()) {
        setPinError("Today's codes haven't been generated by Admin.");
        setIsVerifying(false);
        return;
      }

      const codes = codesDoc.data();
      if (codes[selectedUser.id] === pin) {
        // Valid PIN
        setLoggedInUser(selectedUser);
        setSelectedUser(null);
        setPin('');
      } else {
        setPinError('Incorrect PIN for today.');
      }
    } catch (err) {
      console.error(err);
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm transition-all">
          ...
          >
            ...
            <div className="flex flex-col items-center mb-8">
              ...
              <p className="text-xs font-bold text-[#A0AEC0] uppercase tracking-wider mt-1">Enter your daily code</p>
            </div>
