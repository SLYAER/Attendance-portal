...
export default function Kiosk({ onOpenAdmin, isAdmin }: KioskProps) {
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!todayAttendance) {
        if (!serverCode) {
           setPinError('No daily attendance code generated by admin.');
           setIsVerifying(false);
           return;
        }

        if (String(pin).trim() !== String(serverCode).trim()) {
        ...
        const q = query(collection(db, 'attendance'), where('userId', '==', selectedUser.id), where('date', '==', autoClockInDateStr));
        const snap = await getDocs(q);
        ...
        if (!selectedUser.password) {
           await updateDoc(doc(db, 'users', selectedUser.id), { password: String(pin).trim() });
           selectedUser.password = String(pin).trim();
        } else {
           if (String(pin).trim() !== String(selectedUser.password).trim()) {
