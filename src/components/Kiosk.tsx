...
export default function Kiosk({ onOpenAdmin, isAdmin }: KioskProps) {
  ...
  const handleUserSelect = async (user: any) => {
    ...
    try {
      ...
      const q = query(collection(db, 'attendance'), where('userId', '==', user.id), where('date', '==', todayStr));
      ...
      if (codeDoc.exists()) {
        const dCodes = codeDoc.data();
        setDailyCode(dCodes[user.id] || null);
      } else {
        setDailyCode(null);
      }
    } catch (e) {
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let serverCode = null;
      if (codeDoc.exists()) {
        serverCode = codeDoc.data()[selectedUser.id] || null;
      }

      let isAuthValid = false;
      const isDailyCodeMatch = serverCode && String(pin).trim() === String(serverCode).trim();
      const isPasswordMatch = selectedUser.password && String(pin).trim() === String(selectedUser.password).trim();
      
      if (isDailyCodeMatch || isPasswordMatch) {
         isAuthValid = true;
      }

      if (!isAuthValid && !selectedUser.password) {
         // Allow setting the password if one is not yet set
         isAuthValid = true;
         await updateDoc(doc(db, 'users', selectedUser.id), { password: pin });
         selectedUser.password = pin; 
      }

      if (!isAuthValid) {
         setPinError('Incorrect password or daily code.');
      ...
      if (!todayAttendance) {
        ...
        await setDoc(newDocRef, {
          ...
          date: autoClockInDateStr,
          clockIn: isoStr,
          status: autoClockInNow.getHours() >= 13 ? 'half-day' : 'present',
        });
      }
      ...
      console.error(err);
      setPinError('Error verifying code.');
    } finally {
      setIsVerifying(false);
    }
  };
      ...
      {selectedUser && (
        <div id="pin-modal" className="absolute inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm transition-all">
          ...
          >
            ...
            <div className="flex flex-col items-center mb-8">
              ...
              <p className="text-xs font-bold text-[#A0AEC0] uppercase tracking-wider mt-1">
                {checkingAttendance ? "Checking status..." 
                  : (selectedUser.password ? "Enter Password or Daily Code" : "Create Password or enter Daily Code")
                }
              </p>
            </div>
            ...
            <button
              type="submit"
              disabled={isVerifying || checkingAttendance || pin.length !== 4}
